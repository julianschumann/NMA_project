...
def neg_log_likelyhood_train(X,y,beta,iterations):
    ...
    for i in range(iterations):
        G,L=logistic_gradient(X,y,beta,theta)
        Loss.append(L)
        m=betam*m+(1-betam)*G
...
def cross_validate(X,y,beta,k):
    ...
    for i in range(k):
        ...
        X_val=X[Index_val,:]
        y_val=y[Index_val]
        theta=neg_log_likelyhood_train(X_train,y_train,beta,2500)
...
def logistic_regression(X,y):
    ...
    return y_pred,beta_opt,np.max(accuracies)
...
if make_data:
    ...
    for data_set in range(39):
        ...
        y_pred,beta,accuracy=logistic_regression(X,y)
        
        np.save('Trial_outcome_pred_{}.npy'.format(data_set),y_pred)
        np.save('Accuracy_val_{}.npy'.format(data_set),accuracy)
        np.save('C_{}.npy'.format(data_set),beta)
        plt.figure()
    ...
    accuracy_cross_val=[]
    for data_set in range(39):   
        Beta.append(np.load('C_{}.npy'.format(data_set)))
        y=np.load('Trial_outcome_{}.npy'.format(data_set))
        accuracy_cross_val.append(np.load('Accuracy_val_{}.npy'.format(data_set)))
        y=np.maximum(y,0)
        y_pred=np.load('Trial_outcome_pred_{}.npy'.format(data_set))
