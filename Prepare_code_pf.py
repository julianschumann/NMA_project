...
if make_data:
    ...
    for i,name in enumerate(Names):
        ...
        X1=np.load('Pupil_area_'+name+'.npy')
        X2=np.load('Face_energy_'+name+'.npy')
        #normalize
        
        X1=(X1-np.mean(X1,1)[:,np.newaxis])/np.std(X1,1)[:,np.newaxis]
        X2=(X2-np.mean(X2,1)[:,np.newaxis])/np.std(X2,1)[:,np.newaxis]
        ...
        np.save('R_pf/Trial_outcome_pred_'+name+'.npy',y_pred)
        np.save('R_pf/Accuracy_val_'+name+'.npy',accuracy)
        np.save('R_pf/C_'+name+'.npy',beta)
        np.save('R_pf/Theta_'+name+'.npy',beta)        
        plt.figure()
    ...
    for i,name in enumerate(Names):  
        Beta.append(np.load('R_pf/C_'+name+'.npy'))
        accuracy_cross_val.append(np.load('R_pf/Accuracy_val_'+name+'.npy'))
        y=np.load('Trial_outcome_'+name+'.npy')
        y=np.maximum(y,0)
        y_pred=np.load('R_pf/Trial_outcome_pred_'+name+'.npy')
