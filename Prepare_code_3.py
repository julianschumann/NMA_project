...
def simple_data(spike, response_time, stim_time):
    ...
    mean_spike_count=(mean_spike_count-np.mean(mean_spike_count,1)[:,np.newaxis])/np.std(mean_spike_count,1)[:,np.newaxis]
...
def logistic_predictor(X,theta):
    ...
    z=np.minimum(np.abs(z),1e7)*np.sign(z)
    sigma=1/(1+np.exp(-z))
...
if make_data:
    ...
    for data_set in range(39):
        ...
        np.save('R3/Trial_outcome_pred_{}.npy'.format(data_set),y_pred)
        np.save('R3/Accuracy_val_{}.npy'.format(data_set),accuracy)
        np.save('R3/C_{}.npy'.format(data_set),beta)
        np.save('R3/Theta_{}.npy'.format(data_set),beta)
    ...
    accuracy_cross_val=[]
    for data_set in range(39):   
        Beta.append(np.load('R3/C_{}.npy'.format(data_set)))
        y=np.load('Trial_outcome_{}.npy'.format(data_set))
        accuracy_cross_val.append(np.load('R3/Accuracy_val_{}.npy'.format(data_set)))
        y=np.maximum(y,0)
        y_pred=np.load('R3/Trial_outcome_pred_{}.npy'.format(data_set))
