...
if make_data:
    ...
    for data_set in range(39):
        ...
        np.save('R1/Trial_outcome_pred_{}.npy'.format(data_set),y_pred)
        np.save('R1/Accuracy_val_{}.npy'.format(data_set),accuracy)
        np.save('R1/C_{}.npy'.format(data_set),beta)
        np.save('R1/Theta_{}.npy'.format(data_set),beta)
        plt.figure()
    ...
    accuracy_cross_val=[]
    for data_set in range(39):   
        Beta.append(np.load('R1/C_{}.npy'.format(data_set)))
        y=np.load('Trial_outcome_{}.npy'.format(data_set))
        accuracy_cross_val.append(np.load('R1/Accuracy_val_{}.npy'.format(data_set)))
        y=np.maximum(y,0)
        y_pred=np.load('R1/Trial_outcome_pred_{}.npy'.format(data_set))
